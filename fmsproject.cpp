...
class Event{
    ...
    friend bool operator==(const Event& e1 , const Event& e2);
    public:
        enum EvtType {arrival, departure1, departure2, departure3};//departure1 = arrival3
        Event(){}
...
//global variables
double clocktime, meanInterArrivalTime, meanServiceTime1, meanServiceTime2, meanServiceTime3, sigma1, sigma2, sigma3, lastEventTime1, lastEventTime2, lastEventTime3, totalBusy1, totalBusy2, totalBusy3, maxQueueLength12, maxQueueLength3, sumResponseTime;

long numberOfCustomers, queueLength12, queueLength3, numberInService1, numberInService2, numberInService3, totalCustomers, numberOfDepartures1, numberOfDepartures2, numberOfDepartures3;

priority_queue <Event> futureEventList;

queue <Event> customers12, customers3;
...
#define PI 3.1415927
    ...
    if(numNormals == 0){
        double r1 = unif();
        double r2 = unif();
        returnNormal = sqrt(-2*log(r1))*cos(2*PI*r1);
...
{
    double serviceTime;
    while ((serviceTime = normal(meanServiceTime1,sigma1))<0);
...
{
    double serviceTime;
    while ((serviceTime = normal(meanServiceTime2,sigma2))<0);
...
void scheduleDeparture3(long RID){
    double serviceTime;
    while ((serviceTime = normal(meanServiceTime2,sigma3))<0);
...
void processDeparture1(Event evt)
{
    ...
    customers12.pop();
    // if(queueLength12 > 0 && customers12.front().get_serverChosen() == "s1") scheduleDeparture1(serialID++);
    if(queueLength12 > 0 && customers12.front().get_serverChosen() == "s1") scheduleDeparture1(customers12.front().get_ID());
    else numberInService1 =0;
    double Response = (clocktime -finished1.get_time());
...
void processDeparture2(Event evt)
{
    ...
    customers12.pop();
    if(queueLength12>0 && customers12.front().get_serverChosen() == "s2") scheduleDeparture2(customers12.front().get_ID());
...
void reportGeneration(){
    ...
    cout<<'\t'<<"STANDARD DEVIATION OF SERVICE TIME FOR SERVER 1: "<<sigma1<<endl;
    cout<<'\t'<<"STANDARD DEVIATION OF SERVICE TIMES FOR SERVER 2: "<<sigma2<<endl;
    cout<<'\t'<<"STANDARD DEVIATION OF SERVICE TIMES FOR SERVER 3: "<<sigma3<<endl;
...
int main(int argc , char* argv[]){
    ...
    meanServiceTime1= 3.1; meanServiceTime2= 2.68; meanServiceTime3= 2.55;
    sigma1= 0.6; sigma2= 0.7; sigma3 = 0.5;
    totalCustomers=50;
    initialization();
